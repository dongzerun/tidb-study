...
```
#### 修改源码
修改源码，使得每次开启事务时打印 `hello transaction`, 对于显示调用事务，修改 [executor/simple.go](https://github.com/pingcap/tidb/blob/master/executor/simple.go#L552) 函数 `executeBegin`, 使得在每次运行 `start transaction` 或是 `begin` 语句时打印日志
```shell
